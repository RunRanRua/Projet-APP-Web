...
function loadData(year, month, day) {
    ...
    fetch(`../php/data_endpoint2.php?date=${year}-${month + 1}-${day}`)
        .then(response => {
            if (!response.ok) {
...
function updateChart(data) {
    ...
    const displayText = `Moyenne : ${data.average.toFixed(2)} dBm, ` +
                        `Max : ${data.max} dBm Min : ${data.min} dBm`;
